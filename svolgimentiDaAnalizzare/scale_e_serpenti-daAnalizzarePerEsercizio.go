...
1. La descrizione del grafo è chiara e corretta? In caso contrario, come la correggereste/chiarireste?
	gli errori sono che:
	1) è che il grafo è connesso dato che da ogni casella posso raggiungere una qualsiasi casella tirando una dato che mi fa spostare di 1-6 caselle
	2) c'è una imprecisione nel nome dei vertici, anzi che pedine andrebbero chiamate caselle o posizione
2. Il grafo descritto modella appropriatamente la situazione descritta nella traccia dell’esercizio?
	si ma potrebbe essere migliorata, la modellazione del grafo è appropriata dato che abbiamo un grafo che offre un "mondo" nel quale operare,
	le casistiche di spostamento potrebbero essere fatte con una lista di adiacenza (ad ogni casella associo una lista di mosse che va dalla mossa più corta alla mossa che mi porta più lontano)
3. Il commento contiene una formulazione del problema da risolvere il termini di grafi? Se sì, quale è questa formulazione? Se no, come lo formulereste?
	no, perchè non sfruttiamo appropriatamente gli archi. Questo può essere fatto operando direttamente sulla lista di adiacenza e non usando una pila
4. Il commento contiene una descrizione dell’algoritmo per risolvere il problema (poi implementato nel programma)? Se sì, dove si trova questa descrizione?
	si, questa descrizione è data nei vari punti nella sezione "approccio al problema"
...
1. Come sono rappresentati nel programma i vertici e gli archi del grafo? Sono rappresentazioni implicite o esplicite?
	Sono rappresentazioni implicite: cells = make(map[int]int), ma secondo me an
